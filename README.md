...
```mermaid
graph TD
    A[用户按下自定义快捷键] --> B[1. 捕捉全部监视器并显示全屏截图遮罩]
    B --> C[2. 用户拖拽框选文字区域]
    ...
    G --> H1[分支 A: 按 Ctrl + C 或点击复制]
    H1 --> I1[1. 写入选中/全部文本到剪贴板]
    I1 --> I2[2. 自动关闭结果窗口]
    
    G --> H2[分支 B: 按 Enter 或点击搜索]
    H2 --> I3[1. 合并多行文本并清理格式]
    I3 --> I4[2. 调用默认浏览器拉起搜索引擎搜索]
    I4 --> I2
    
    I2 --> J[3. 激活 2 分钟空闲定时器]
    J -- 无新操作 --> K[4. 回收引擎资源并调用 EmptyWorkingSet]
    K --> L[后台物理内存回归至 ~20MB]
```
